...
const Search = () => {
  ...
  const [appliedFilters, setAppliedFilters] = useState({
    minPrice: 5000, maxPrice: "", category: "", region: ""
  });

  const [mounted, setMounted] = useState(false);
  const { user } = useStore()

  const searchResult = useCachedQuery(api.homes.filterHomes, {
  ...
  const count = useCachedQuery(api.messages.countUnreadMessages, user?._id ? { userId: user._id } : "skip", `cache_unread_count_${user?._id}`) ?? 0;
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className='w-full'>
        ...
        {Category !== "All" && <TouchableOpacity activeOpacity={0.7} onPress={applyFilter} className={` bg-[#124BCC] flex flex-row items-center justify-center py-4 px- mx-8 rounded-md  mb-4`} >
          ...
          <Text className={`text-white font-bold `}>{t("Filter")} {Category}</Text>
        </TouchableOpacity>}
      </View>
      <View className='w-full'>
            ...
            return (
              <TouchableOpacity onPress={() => router.push(`/House/${item._id}`)} activeOpacity={0.7} className='flex flex-row items-center bg-white p-3 rounded-lg shadow-sm shadow-gray-400 justify-between mx-2 my-1'>
                <View className="w-full flex flex-row justify-between items-center ">
                  <Image source={{ uri: item.home_cover }}
